...
class Cardstream extends PaymentModule
{
	...
	protected function createTable()
	{
        ...
        CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'cardstream_wallets` (
...
');
	...
	public function uninstall()
	{
		...
		return (Configuration::deleteByName('PAYMENT_NETWORK_MERCHANT_ID') &&
	...
	{
		return Db::getInstance()->execute('
        DROP TABLE IF EXISTS `' . _DB_PREFIX_ . 'cardstream_wallets`;
    ');
	}
...
SCRIPT;
	...
	public function captureOrder($order)
	{
		...
		if (
			...
			$wallet_table_name = _DB_PREFIX_ . 'cardstream_wallets';
	...
	public function onSuccess($response)
	{
		...
		$link = sprintf(
			'%sindex.php?controller=order-confirmation&id_cart=%s&id_module=%s&id_order=%s&key=%s',
			$base_url,
			$order->id_cart,
			$this->id,
			$response['orderRef'],
